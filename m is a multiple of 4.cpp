...
int** Tile;  // an m * m grid
int* selected;  // S(of the paper)
...
int main() {
	...
	cout << "This program will try to help you construct a set of O-tile strucutres, where each O-tile structure has m*m/4-m pieces of 2 * 2 tiles, m pieces of 1 * 2 tiles and m pieces of 2 * 1 tiles for m ¡Ô0 (mod 4) and 4 <= m <= 32." << endl;
	cout << "(1) m*m/4-m pieces of 2 * 2 tiles M_{i,j} * N_{i,j}, where M_{i,j} = {j, j+l_{i}}, N_{i,j} = {p*j+a_{i}, p*j+a_{i}+l_{i}}, 0 <= i <= m/4-2, 0 <= j <= m-1." << endl;
	...
	cout << "\nPlease enter m: ";
	cin >> m;
	s_max = m * m / 4 + m;  // each constructed O-tile structure contains s_max pieces of tiles
	grp = m / 4 - 1;  // m*m/4-m pieces of 2 * 2 tiles are divided into grp different groups, each group contains m pieces of 2 * 2 tiles
	...
	if (m == 4) {  // the case of m = 4 is different from other values, since 4 * 4 grids do not contain 2 * 2 tiles
		p = 3;  // p = 1 is equivalent to p = 3
		
		count4 = 0;

		for (int i = 0; i < 3; i++) {  // there are six possibilities for the column indices of 1 * 2 tiles
			for (int j = i + 1; j < 4; j++) {
				combo[count4][0] = i;
				combo[count4][1] = j;
				count4++;
		...
		for (int i = 0; i < 6; i++) {  // loop through six possibilities
			for (int j = 0; j < m; j++) {  // 1 * 2 tiles 
				Tile[j][(p * j + combo[i][0]) % m] = Tile[j][(p * j + combo[i][1]) % m] = (j + 1) + grp * m;
			}

			for (int j = 0; j < m; j++) {  // 2 * 1 tiles 
				Tile[mod((inv(p, m) * (j - combo[5 - i][0])), m)][j] = (j + 1) + (grp + 1) * m;
				Tile[mod((inv(p, m) * (j - combo[5 - i][1])), m)][j] = (j + 1) + (grp + 1) * m;
			}
		...
		for (p = 3; p < m - 2; p++) {  // p != 1, since when p = 1, 2 * 2 tiles will overlap, the same as p = m - 1
			if (gcd(p, m) == 1) {
				permuandcombi1 combi(1, m / 2 + 1, grp);
				do {  // iteratively pick numbers len_0 < len_1 < ... < len_{grp-1} from the set {1, 2, ... ,m / 2}
					permuandcombi1 permu(0, m, grp);
					do {  // iteratively pick distinct numbers {arr_0, arr_1, ... , arr_{grp-1}} from the set {0, 1, ... ,m - 1}
						if (checkArr(permu.arr, combi.len)) {
							count1 = 0;
							for (int i = 0; i < grp; i++) {
								for (int j = 0; j < m; j++) {  // 2 * 2 tiles
...
bool checkLen(int* l1, int b, int c, int d) {
	for (int i2 = 0; i2 < grp; i2++) {
		if (l1[i2] == c - b || (l1[i2] + (c - b)) == m || l1[i2] == d || (l1[i2] + d) == m) {
			return false;
		}
	}
	return true;
...
void writeO_Tile(int* a1, int* l2, int i3, ofstream& fout) {
	...
	if (m == 8) {
		fout << "The value of len is: {" << l2[0] << "}" << endl;
	}
	else if (m > 8) {
		for (int v3 = 0; v3 < grp; v3++) {
			if (v3 == 0) {
				fout << "The value of len is: {" << l2[v3] << ", ";
			}
			else if (v3 == grp - 1) {
				fout << l2[v3] << "}";
			}
